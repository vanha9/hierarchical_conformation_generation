...
class ProjectedDecoder(StructureTokenDecoder):
    ...
    def decode(
        ...
        weight_front, weight_special = embed_weight[:4096], embed_weight[4096:]
        padding = torch.zeros(4096 - decode_proj_weight.shape[1], embed_weight.shape[1]).to(decode_proj_weight.device)
        new_embed_weight = torch.cat([decode_proj_weight.T @ weight_front, padding, weight_special])

        projected_embed = torch.nn.Embedding(embed_weight.shape[0], embed_weight.shape[1])
        projected_embed.weight = nn.Parameter(new_embed_weight.clone())
...
class ProjectedCodebookModel(nn.Module):
    ...
    def forward(self, coords, attention_mask, sequence_id, residue_index, ss_len):
        chain = ProteinChain.from_atom37(
            coords, sequence=None
        )
        #gt_bb_coords = coords[..., :3, :]
        gt_bb_coords = coords[..., :ss_len, :3, :]
        ...
        structure_tokens = structure_tokens[:ss_len]
...
def train():
    ...
    for epoch in range(5):
        ...
        #for step, data in enumerate(pbar):
            ...
            # Build sequence_id
            L1, L2 = atom_pos_aa.shape[1], atom_pos_ss.shape[1]
            res_id1 = torch.arange(L1)

            start = data2["start"].float().clone().detach()
            end = data2["end"].float().clone().detach()
            res_id2 = torch.round((start + end) / 2).long().squeeze(0)
            
            res_id = torch.cat([res_id2, res_id1]).unsqueeze(0).to(device)
            #L = atom_pos_ss.shape[1]
            #res_id = torch.arange(L).unsqueeze(0).to(device)

            optimizer.zero_grad()
            bb_coords, commitment_loss, bb_dist_loss, bb_direction_loss = model(atom_pos, None, None, res_id, L2)
        ...
        decode_proj_weight = model.decode_proj_weight.detach().cpu()

        torch.save(decode_proj_weight, os.path.join(save_dir, f"proj_weight_enc_epoch{epoch}.pt"))
        torch.save(encode_proj_weight, os.path.join(save_dir, f"proj_weight_dec_epoch{epoch}.pt"))
