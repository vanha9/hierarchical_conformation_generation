    ...
    def forward(
        ...
        if self.use_plain_attn:
            ...
            if attn_mask ==None:
                ...
                r1 = self.attn(x, sequence_id, attn_mask= attn_mask.unsqueeze(1).expand(-1, self.n_heads, -1, -1))
            x = x + r1 / self.scaling_factor

        if self.use_geom_attn:
            #attn_mask = attn_mask.unsqueeze(1).expand(-1, self.v_heads, -1, -1) 
            if attn_mask ==None:
                r2 = self.geom_attn(x, frames, frames_mask, sequence_id, chain_id)
            else:
                r2 = self.geom_attn(x, frames, frames_mask, sequence_id, chain_id, attn_mask= attn_mask.unsqueeze(1).expand(-1, self.v_heads, -1, -1) )
