...
def main(samples_dir, reference_dir, output_dir, task='apo'):
    ...
    if task == 'apo':
        ...
        exclude_names = ['1akz.A.pdb', '1zol.A.pdb', '1hw1.B.pdb']
        data_df = data_df.drop(index=exclude_names, errors='ignore')
        
    else:
        data_df = pd.read_csv(reference_dir / 'splits' / 'codnas.csv', index_col='name').sort_index()
        exclude_names = ['3njq.A.pdb','3tp2.A.pdb','5fhc.J.pdb','3r9j.C.pdb','5hk5.H.pdb','4qhf.A.pdb','1uxm.K.pdb','1mnm.C.pdb']
        data_df = data_df.drop(index=exclude_names, errors='ignore')
