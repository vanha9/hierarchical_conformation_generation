...
class ESMOutput:
    ...
    fine_structure_logits: torch.Tensor = None
    coarse_structure_logits:torch.Tensor= None
    mid_structure_logits:torch.Tensor= None
    structure_logits:torch.Tensor= None
    secondary_structure_logits: torch.Tensor= None
    sasa_logits: torch.Tensor= None
    function_logits: torch.Tensor= None
    residue_logits: torch.Tensor= None
    coarse_embeddings: torch.Tensor= None
    mid_embeddings: torch.Tensor= None
    fine_embeddings: torch.Tensor= None
...
class EncodeInputs(nn.Module):
    ...
    def __init__(self, d_model: int):
        ...
        # Structure
        self.MaskC_structure_token_fc = nn.Linear(1280, d_model)
        self.CM_structure_token_fc = nn.Linear(1280, d_model)
        self.MF_structure_token_fc = nn.Linear(1280, d_model)
        
        self.MaskC_structure_tokens_embed = nn.Embedding(32 + 5, 1280)
        self.CM_structure_tokens_embed = nn.Embedding(32 + 5, 1280)
        self.MF_structure_tokens_embed = nn.Embedding(512 + 5, 1280)
        self.MaskC_structure_tokens_embed.reset_parameters()
        self.CM_structure_tokens_embed.reset_parameters()
        self.MF_structure_tokens_embed.reset_parameters()
    ...
    def forward(
        ...
        var_structure_tokens: torch.Tensor,
        coarse_structure_tokens: torch.Tensor,
        fine_structure_tokens: torch.Tensor,
        ...
        if scale == 0:
            coarse_structure_tokens = var_structure_tokens
        elif scale == 2:
            fine_structure_tokens = var_structure_tokens

        if scale == -1 or scale == 0:
            coarse_structure_embed = self.MaskC_structure_token_fc(self.MaskC_structure_tokens_embed(coarse_structure_tokens))
        if scale == -1 or scale == 2:
            fine_structure_embed = self.MF_structure_token_fc(self.MF_structure_tokens_embed(fine_structure_tokens))
        ...
        rest_embed = sequence_embed + plddt_embed + structure_per_res_plddt + ss8_embed + sasa_embed + function_embed + residue_embed 

        if scale == 0 :
            return rest_embed + coarse_structure_embed
        elif scale == 2:
            return rest_embed + fine_structure_embed
        return (rest_embed + coarse_structure_embed, rest_embed + fine_structure_embed)
...
class OutputHeads(nn.Module):
    def __init__(self, d_model: int):
        ...
        self.coarse_structure_head = RegressionHead(d_model, 32)
    ...
    def forward(self, x_32: torch.Tensor,x_4096: torch.Tensor, embed: torch.Tensor) -> ESMOutput:
        sequence_logits = self.sequence_head(x_4096)
        fine_structure_logits = self.structure_head(x_4096)
        coarse_structure_logits = self.coarse_structure_head(x_32)     
        secondary_structure_logits = self.ss8_head(x_4096)
        sasa_logits = self.sasa_head(x_4096)
        function_logits = self.function_head(x_4096)
        ...
        residue_logits = self.residue_head(x_4096)

        return ESMOutput(
            sequence_logits=sequence_logits,
            fine_structure_logits=fine_structure_logits,
            coarse_structure_logits=coarse_structure_logits,
